...
RUN apt-get update && \
    apt-get install -y git dpkg-dev cmake g++ gcc binutils libx11-dev libxpm-dev libxft-dev libxext-dev libgsl0-dev gfortran libfftw3-dev libxml2-dev wget python
    
RUN mkdir /usr/local/share/pythia && \
...
RUN mkdir /usr/local/share/root && \
